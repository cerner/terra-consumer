...
const ProfileLinks = ({
  ...
  const linkElements = linkItems.map((linkItem) => {
    const linkElement = (<a href={linkItem.uri} onClick={() => window.location = linkItem.uri} className={cx('suppress-hyperlink')}>
      <div key={`${linkItem.text}`} className={cx('link', 'divider-bottom')}>{linkItem.text}</div></a>);
    return linkElement;
  });
